...
bool TargaTextureClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* context, LPCSTR fileName)
{
	int width = 0;
	int height = 0;
...
void TargaTextureClass::Shutdown()
{
	if (textureView)
	{
		textureView->Release();
...
bool TargaTextureClass::LoadTarga(LPCSTR fileName, int& height, int& width)
{
